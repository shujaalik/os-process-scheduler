...
const GanttChart = ({ ganttChartInfo }: GanttChartProps) => {
  ...
  useLayoutEffect(() => {
    function updateSize() {
      ...
      setContainerWidth(containerEl?.current?.offsetWidth || 0);
    }
  ...
  const maxTimeItemCount = ~~((containerWidth || 0) / itemWidth);
        ...
        <>
          {Array.from({ length: numberOfLines }).map((_, ind) => {
            if (ind === numberOfLines - 1 && lastLineItemCount !== 0) {
              ...
              const prevCounter = timeCounter;
              timeCounter += maxTimeItemCount - 1;
              const prevJobCounter = jobCounter;
